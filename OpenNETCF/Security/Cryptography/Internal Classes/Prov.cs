...
namespace OpenNETCF.Security.Cryptography.Internal
{
	internal class Prov
	{
		public static void SetProvParam(IntPtr prov, ProvParamSet param, byte [] data)
		{
            ...
            bool retVal = NativeMethods.CryptSetProvParam(prov, (uint)param, data, flags);
		...
		public static ArrayList GetProvParam(IntPtr prov, ProvParamEnum param, uint dataLen)
		{
			...
			while ( true )
			{
                ...
                bool retVal = NativeMethods.CryptGetProvParam(prov, (uint)param, data, ref dataLen, flags);
		...
		public static byte [] GetProvParam(IntPtr prov, ProvParam param)
		{
			...
			if(ec == ErrCode.MORE_DATA)
			{
                ...
                retVal = NativeMethods.CryptGetProvParam(prov, (uint)param, data, ref dataLen, flags);
		...
		{
			uint reserved = 0;
            bool retVal = NativeMethods.CryptSetProviderEx(provName, (uint)provType, ref reserved, (uint)provDefFlag);
			ErrCode ec = Error.HandleRetVal(retVal);
		}

		public static void SetProvider(string provName, ProvType provType)
		{
            bool retVal = NativeMethods.CryptSetProvider(provName, (uint)provType);
			ErrCode ec = Error.HandleRetVal(retVal);
		}
		...
		public static string GetDefaultProvider(ProvType provType, ProvDefaultFlag provDefFlag)
		{
            ...
            bool retVal = NativeMethods.CryptGetDefaultProvider((uint)provType, ref reserved, flags, provName, ref dataLen);
			ErrCode ec = Error.HandleRetVal(retVal, ErrCode.MORE_DATA);
			if(ec == ErrCode.MORE_DATA)
			{
				provName = new StringBuilder((int)dataLen);
                retVal = NativeMethods.CryptGetDefaultProvider((uint)provType, ref reserved, flags, provName, ref dataLen);
				ec = Error.HandleRetVal(retVal);
			}
		...
		private static ProviderInfo [] EnumProviders(out uint [] provTypes)
		{
			...
			while ( true )
			{
                bool retVal = NativeMethods.CryptEnumProviders(dwIndex, ref dwReserved, dwFlags, ref dwProvType, sProv, ref cbProvName);
				ErrCode [] eca = new ErrCode[]{ErrCode.NO_MORE_ITEMS, ErrCode.MORE_DATA};
				ErrCode ec = Error.HandleRetVal(retVal, eca);
				if ( !retVal )
				{					
                ...
                retVal = NativeMethods.CryptEnumProviders(dwIndex, ref dwReserved, dwFlags, ref dwProvType, sProv, ref cbProvName);
				ec = Error.HandleRetVal(retVal);
				dwIndex = dwIndex + 1;
				cbProvName = 0;
