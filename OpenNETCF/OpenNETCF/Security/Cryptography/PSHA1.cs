...
namespace OpenNETCF.Web.Services2.Security.Cryptography
{
	public class PSHA1
	{
		...
		*/
		...
		{
            byte[] baLabel = OpenNETCF.Security.Cryptography.Internal.Format.GetBytes(label);
			byte [] baTimestamp = new byte[0]; //not passing a timestamp
		...
		{
            byte[] baPassword = OpenNETCF.Security.Cryptography.Internal.Format.GetBytes(password);
            byte[] baLabel = OpenNETCF.Security.Cryptography.Internal.Format.GetBytes(label);
			byte [] baNonce = new byte[0];
			if(nonce != null && nonce != String.Empty)
                baNonce = OpenNETCF.Security.Cryptography.Internal.Format.GetB64(nonce);
            byte[] baTimestamp = OpenNETCF.Security.Cryptography.Internal.Format.GetBytes(timestamp);
