	...
	throw new Exception("This is an NDOC Build, not a build for use!");
		...
		public RiffStream RecordFor(Stream st, short Seconds, WaveFormat2 format)
		{
			...
			for ( int i = 0; i < 2; i ++ )
			{
                ...
                CheckWaveError(NativeMethods.waveInAddBuffer(m_hWaveIn, hdr.Header, hdr.HeaderLength));
			}
		...
		private WaveHeader GetNewRecordBuffer(int dwBufferSize)
		{
			...
			// prepare the header
            CheckWaveError(NativeMethods.waveInPrepareHeader(m_hWaveIn, hdr.Header, hdr.HeaderLength));
			return hdr;
		}
		...
		private void m_recmw_WaveDoneMessage(object sender, IntPtr wParam, IntPtr lParam)
		{
            ...
            CheckWaveError(NativeMethods.waveInUnprepareHeader(m_hWaveIn, hdr.Header, hdr.HeaderLength));
			hdr.RetrieveHeader();
