...
namespace OpenNETCF.IO.FileSystem
{
	/// <summary>
	...
	public class FileSystemMonitor : Component, IDisposable		
	{
		...
		public FileSystemMonitor(string path): this()
		{
			this.Path = path;
		}
		...
		public FileSystemMonitor(string path, string filter): this()
		{
			this.Path = path;
			this.Filter = filter;
		...
		#region WindowSink
        public class FileNotificationMonitor  : IDisposable
		{
            ...
            private void RaiseEvents(FileAction action, string fileName)
            {
                ...
                switch (action)
                {
                    ...
                    //    if ((m_fsw.notifyFilter & NotifyFilters.DirectoryName) == NotifyFilters.DirectoryName)
                    //    {
                    //        args = new FileSystemEventArgs(WatcherChangeTypes.Created, m_path, fileName);
                    //        m_fsw.OnCreated(args);
                    //    }
