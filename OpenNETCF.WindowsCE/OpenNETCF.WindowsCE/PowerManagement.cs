...
namespace OpenNETCF.WindowsCE
{
  ...
  public static class PowerManagement
  {
    ...
    /// <summary>
    /// Forces the device into On state.  Useful for forcing a suspended device to stay on
    /// </summary>
    public static void SetOnState()
    {
        int ret = NativeMethods.SetSystemPowerState(IntPtr.Zero, PowerStateFlags.On, NativeMethods.POWER_FORCE);
        if (ret != NativeMethods.ERROR_SUCCESS)
        {
            throw new Win32Exception(ret);
        }
