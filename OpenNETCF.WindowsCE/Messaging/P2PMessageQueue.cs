...
namespace OpenNETCF.WindowsCE.Messaging
{
	...
	public class P2PMessageQueue 
    {
		...
		public P2PMessageQueue(bool forReading, string name, int maxMessageLength, int maxMessages, out bool createdNew) : this(forReading, name, maxMessageLength, maxMessages)
        {
        ...
        {
            IntPtr h = NativeMethods.OpenMsgQueue(processHandle, queueHandle, new MsgQueueOptions(forReading));
        ...
        private object m_sendSyncRoot = new object();
		/// <summary>
		/// Adds a message to the queue.
		/// The queue must have been created with forReading = false.
		/// </summary>
			...
			while (!hQue.Equals(InvalidHandle))
            {
                ...
                if (res == NativeMethods.WAIT_OBJECT_0)
                {
					if (DataOnQueueChanged != null)
                    {
                        if (hQue.Equals(InvalidHandle))
                        {
                            // in case the queue became invalid after the wait was set - it's a small
                            // window, but it's possible (and maybe has happened in production)
                            return;
                        }
