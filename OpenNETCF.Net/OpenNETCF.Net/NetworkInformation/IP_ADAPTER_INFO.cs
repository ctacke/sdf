...
namespace OpenNETCF.Net.NetworkInformation
{
  internal class IP_ADAPTER_INFO
  {
    ...
    public unsafe GatewayIPAddressInformationCollection GatewayList
    {
      get
      {
        UnicastIPAddressInformationCollection collection = new UnicastIPAddressInformationCollection(this.AdapterName);
        fixed (byte* p = m_data)
        {
          byte* pAddressList = p;
          pAddressList += GatewayListOffset;
          collection = IPUtility.ParseAddressesFromPointer(pAddressList, this.AdapterName);
        }
